...
mod tests {
    ...
    #[test]
    fn wait_timeout_huge_timeout_returns_deadline_overflow() {
    ...
    #[test]
    fn wait_timeout_notify_without_trigger_keeps_original_deadline() {
        let (_g, s) = ShutdownSignal::new();
        let waiter = {
            let signal = s.clone();
            // 起点在 waiter 线程内记录：把「spawn 后延迟调度」的时间排除在上界之外，
            // 只约束 wait_timeout 从自身起点到返回的耗时，确保原 deadline 不被重算。
            thread::spawn(move || {
                let start = std::time::Instant::now();
                let result = signal.wait_timeout(Duration::from_millis(80));
                (result, start.elapsed())
            })
        };
        thread::sleep(Duration::from_millis(10));
        // 未置 triggered 的 notify 等价于伪唤醒；不得误报触发，也不得重算 deadline。
        s.inner.cv.notify_all();
        let (result, elapsed) = waiter.join().expect("waiter");
        assert_eq!(result, Ok(false));
        assert!(elapsed >= Duration::from_millis(50));
        // 上界：原 deadline 80ms + slack，避免伪唤醒后无限等或重算更长 deadline。
        const SLACK: Duration = Duration::from_millis(80);
        assert!(
            elapsed <= Duration::from_millis(80) + SLACK,
            "elapsed={elapsed:?} 超过原 80ms deadline + slack"
        );
    }
